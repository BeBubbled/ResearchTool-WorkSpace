#!/usr/bin/env bash
# Start the local web panel on macOS or Linux.
# Usage: ./run_web_panel.sh [--no-browser] [--no-pause] [--port 8765]
...
usage() {
    cat <<'EOF'
Usage: ./run_web_panel.sh [options]
...
validate_port() { [[ "$1" =~ ^[0-9]+$ ]] && (( 10#$1 <= 65535 )); }
    ...
    case "$(uname -s)" in
        Darwin)
            if ! command -v brew >/dev/null 2>&1; then
                cat >&2 <<'EOF'
[toolbox] Python 3.10+ was not found. Install it from
https://www.python.org/downloads/macos/ (or install Homebrew), then run again.
EOF
                return 1
            fi
        ...
        Linux)
            cat >&2 <<'EOF'
[toolbox] Python 3.10+ was not found. Install Python 3.10 or newer with your
distribution's package manager, then run this launcher again. Examples:
...
EOF
            ...
            ;;
        *)
            printf '[%s] Unsupported POSIX platform: %s\n' "$PREFIX" "$(uname -s)" >&2
            return 1
            ;;
    esac
    get_system_python
...
tools will be unavailable until they are installed and on PATH.
...
ensure_go() {
    ...
    if [[ "$(uname -s)" == "Darwin" ]] && command -v brew >/dev/null 2>&1; then
        write_step "Go was not found. Installing it with Homebrew." >&2
        brew install go >&2
        command -v go
        return
    fi
    cat >&2 <<'EOF'
[toolbox] Go 1.23+ is required to build the document translation backend.
Install it, then run this launcher again. Examples:
...
ensure_translation_backend() {
    ...
    (cd "$source_dir" && "$go_binary" build -trimpath -o "$binary" ./cmd/translator)
}

ensure_pip() {
    if "$VENV_PYTHON" -c 'import pip' >/dev/null 2>&1; then return; fi
    write_step "Project .venv is missing pip. Repairing it with ensurepip."
    "$VENV_PYTHON" -m ensurepip --upgrade
    "$VENV_PYTHON" -c 'import pip' >/dev/null 2>&1 || {
        printf '[%s] Could not repair pip. Delete .venv and run this launcher again.\n' "$PREFIX" >&2
        return 1
    }
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --port) [[ $# -ge 2 ]] || { printf '[%s] --port requires a value.\n' "$PREFIX" >&2; exit 2; }; PORT="$2"; shift 2 ;;
...
if [[ "$NO_BROWSER" -eq 0 ]]; then write_step "Your browser will open automatically when the panel is ready."; fi
PANEL_STARTED=1
