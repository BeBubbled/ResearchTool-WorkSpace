...
Video mode (extract frames first):
    pip install python-pptx Pillow opencv-python
...
2) Video mode:
python sort_images_ppt.py --video-dir "Videos" --frame-idx 0 3 5 7 11 --rows 3 --cols 5 --cell-size-cm 5 --gap-px 4 --margin-cm 1 --fit fit --out grid_from_videos.pptx
"""
...
try:
    import cv2  # only needed for video mode
except ImportError:
    cv2 = None
...
def parse_args():
    ...
    src.add_argument(
        "--video-dir",
        type=str,
        help="Directory containing videos. Frames will be extracted for each video.",
    )
    ...
    gap.add_argument(
        "--gap-px",
        type=float,
        help="Gap between cells in pixels (96 DPI assumed).",
    )
    ...
    margin.add_argument(
        "--margin-px",
        type=float,
        help="Outer margin in pixels (96 DPI assumed).",
    )
    ...
    p.add_argument(
        "--sort",
        choices=["name", "mtime", "none", "numeric"],
        default="name",
    )
    ...
    p.add_argument(
        ...
        type=int,
        nargs="+",
        help="Frame indices to extract per video, e.g., 0 3 5 7 11.",
    )

    p.add_argument(
        "--out",
        type=str,
        default="grid_output.pptx",
...
def expand_images(args_images: List[str]) -> List[str]:
    ...
    for term in args_images:
        ...
        # 4. 既不是文件也不是目录
        print(
            f"WARNING: '{term}' is not a file or directory, ignored.",
            file=sys.stderr,
        )
...
def extract_frames_from_videos(
    video_dir: str, frame_indices: List[int], tmp_root: str
) -> List[str]:
    if cv2 is None:
        raise SystemExit(
            "OpenCV (cv2) is required for video mode. Install with 'pip install opencv-python'."
        )

    videos = list_videos(video_dir)
    if not videos:
        print("No videos found in directory:", video_dir, file=sys.stderr)
        sys.exit(2)

    frame_indices = sorted(set(frame_indices))
    print("[INFO] Using frame indices per video:", frame_indices)

    all_frames: List[str] = []
    for vid_idx, vpath in enumerate(videos):
        base = os.path.splitext(os.path.basename(vpath))[0]
        print(f"[INFO] Processing video {vid_idx:03d}: {base}")

        cap = cv2.VideoCapture(vpath)
        if not cap.isOpened():
            print(f"WARNING: cannot open video: {vpath}", file=sys.stderr)
            continue
        ...
        for fi in frame_indices:
            if fi < 0 or fi >= total:
                print(
                    f"  WARNING: frame index {fi} out of range for {base}, skip.",
                    file=sys.stderr,
                )
                continue
            ...
            if not ok or frame is None:
                print(
                    f"  WARNING: failed to read frame {fi} from {base}, skip.",
                    file=sys.stderr,
                )
                continue

            out_name = f"{vid_idx:03d}_{base}_f{fi:06d}.png"
            out_path = os.path.join(tmp_root, out_name)
            cv2.imwrite(out_path, frame)
            all_frames.append(out_path)
            print(f"  saved frame {fi} -> {out_name}")

        cap.release()

    print(f"[INFO] Total extracted frames: {len(all_frames)}")
    return all_frames
...
def compute_units(args):
    ...
    if args.cell_size_cm is not None:
        ...
        if args.cell_width_cm is None or args.cell_height_cm is None:
            raise SystemExit(
                "Provide both --cell-width-cm and --cell-height-cm, or use --cell-size-cm."
            )
    ...
    if args.cell_aspect is not None:
        ...
        if abs(ratio - args.cell_aspect) > 1e-3:
            print(
                f"WARNING: Cell aspect {ratio:.3f} != expected {args.cell_aspect:.3f}.",
                file=sys.stderr,
            )
...
def grid_size_cm(
    rows: int, cols: int, cw_cm: float, ch_cm: float, gap_cm: float, margin_cm: float
):
...
def main():
    ...
    # 视频模式: 必须有 frame-idx
    if args.video_dir and not args.frame_idx:
        raise SystemExit(
            "In video mode, you must provide --frame-idx (e.g. --frame-idx 0 3 5 7 11)."
        )

    # 收集图片 (两种模式)
    tmp_dir = None
    if args.video_dir:
        tmp_dir = tempfile.mkdtemp(prefix="frames_for_ppt_")
        print("[INFO] Temporary frame directory:", tmp_dir)
        imgs = extract_frames_from_videos(
            args.video_dir, args.frame_idx, tmp_dir
        )
    else:
    ...
    if len(imgs) < needed:
        print(
            f"WARNING: Need {needed} images, but only {len(imgs)} provided. "
            f"Filling remaining cells with last image.",
            file=sys.stderr,
        )
        if imgs:
    ...
    grid_w_cm, grid_h_cm = grid_size_cm(
        args.rows, args.cols, cw_cm, ch_cm, gap_cm, margin_cm
    )
    ...
    print(
        "Open the PPTX, select shapes (Ctrl/Cmd+A), Group (optional), then copy to your target deck."
    )
