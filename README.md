...
- 侧栏提供独立的“AI、语音与 GitHub 配置”菜单，以标签页统一管理 OpenAI-compatible LLM 预设、Azure Speech 和 Markdown 图片仓库。整篇翻译只能选择这里保存的 LLM 预设；各类 Token/API Key 仅保存在本机 `.env` 与当前后端进程中，不会返回浏览器或写入任务日志。
