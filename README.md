...
- 可点击或拖拽上传 PDF，选择仅 OCR 原文阅读或 OCR 后中英对照翻译；PDF OCR 复用 Mathpix 配置。上传文件仅写入项目本机的 `.runtime/` 任务目录（已被 Git 忽略），不会上传到 GitHub。
- 上传 `.md` 或 `.mmd` 时直接按章节、段落、代码块和展示公式渲染，无需 OCR。
- 划选任意原文或译文后，可直接选择“解释选区、解释公式、公式的几何意义、推导这一步、总结、直觉解释、批判性阅读、实现视角”。系统会注入所在章节和前后两段原文，而非整篇论文。
- 所有 LLM 入口（文档翻译、PDF OCR 后翻译与阅读器划词问答）均可使用 `.env` 中的 OpenAI-compatible LLM 预设，或填写临时配置后“保存为本机预设”。保存后会永久写入项目 `.env` 并立即可用；密钥不会写入文档、任务选项或日志。

MathJax 通过浏览器加载以显示 LaTeX。若本机离线，论文原始公式文本仍会保留，但不会排版为数学公式。
